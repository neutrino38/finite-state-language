defmodule FSL.HTTPTest do
  @moduledoc """
  Tests for the FSL.HTTP scenario mixin (`http_GET` / the Valet coordinator).

  We drive `FSL.HTTP.get_async/4` directly (the layer under the macro) so we
  ...
  test "delivers {tag, {:ok, %Req.Response{}}} on success" do
    FSL.HTTP.get_async("http://stub/ok", 1_000, :prov, ok_adapter("hello"))
  ...
  test "routes the tag: two concurrent requests never cross their replies" do
    FSL.HTTP.get_async("http://stub/a", 1_000, :req_a, ok_adapter("A"))
    FSL.HTTP.get_async("http://stub/b", 1_000, :req_b, ok_adapter("B"))
  ...
  test "a network error comes back as {tag, {:error, exception}}" do
    ...
    FSL.HTTP.get_async("http://stub/down", 1_000, :prov, adapter)
  ...
  test "a worker crash comes back as {tag, {:error, {:crash, reason}}}" do
    ...
    FSL.HTTP.get_async("http://stub/boom", 1_000, :prov, adapter)
  ...
  test "timeout: worker is killed, {:error, :timeout} delivered, NO late message" do
    ...
    coord = FSL.HTTP.get_async("http://stub/slow", 100, :prov, slow)
