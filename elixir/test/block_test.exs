defmodule FSL.BlockTest do
  ...
  defp run(module) do
    ...
    {pid, ref} =
      spawn_monitor(fn ->
        send(watcher, {:outcome, FSL.Runner.run_instance(module)})
      end)
  ...
  test "sbb_fsm in an on_events clause is a compile error" do
    source = """
    defmodule FSL.BlockTest.InClause do
      use FSL.Machine
      state initial_state do
        on_events do
          {:go, _} -> sbb_fsm(FSL.BlockTest.Echo)
        end
      end
    end
    """
  ...
  test "a block whose .beam is on the code path but not yet loaded is still a block" do
    ...
    source = """
    defmodule FSL.BlockTest.NotYetLoaded do
      use FSL.Block
    ...
    FSL.Runner.run_sbb(%FSL.Context{}, module)
  ...
  test "entering something that is not a block is rejected" do
    assert_raise ArgumentError, ~r/is not a service building block/, fn ->
      FSL.Runner.run_sbb(%FSL.Context{}, HostEcho)
    end
  end
  ...
  test "a shutdown reaching a block runs the host's on_shutdown" do
    ...
    {pid, ref} =
      spawn_monitor(fn ->
        send(test_pid, {:outcome, FSL.Runner.run_instance(HostWithShutdown)})
      end)
  ...
  test "a shutdown reaching a block still aborts a host that declares none" do
    ...
    {pid, _ref} =
      spawn_monitor(fn ->
        send(test_pid, {:outcome, FSL.Runner.run_instance(HostNoShutdown)})
      end)
  ...
  test "a return that is not {namespace, outcome, data} is a compile error" do
    assert_raise CompileError, ~r/three elements, the last a map/, fn ->
      compile!("""
      defmodule FSL.BlockTest.TwoElements do
        use FSL.Block
        state initial_state do
  ...
  test "a return under another block's namespace is a compile error" do
    assert_raise CompileError, ~r/namespace is :wrong_ns, not :call/, fn ->
      compile!("""
      defmodule FSL.BlockTest.WrongNs do
        use FSL.Block
  ...
  test "an outcome the block does not declare is a compile error" do
    assert_raise CompileError, ~r/:conected is not one of this block's declared outcomes/, fn ->
      compile!("""
      defmodule FSL.BlockTest.Typo do
        use FSL.Block
        @sbb_namespace :typo
  ...
  test "a block with no declared vocabulary keeps its outcomes unchecked" do
    assert [{_mod, _bin} | _] =
             compile!("""
             defmodule FSL.BlockTest.Undeclared do
               use FSL.Block
               state initial_state do
  ...
  test "a face module can register its blocks' namespaces for the whole scenario" do
    defmodule Face do
      defmacro __using__(_opts) do
        FSL.Machine.register_namespace(__CALLER__.module, :faced)
        quote(do: :ok)
      end
    end
  ...
  test "the per-state try/catch stays transparent to a throw" do
    ...
    assert catch_throw(FSL.Runner.run_instance(Thrower)) == :crossed_the_state_frame
  end
end
