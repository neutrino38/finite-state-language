defmodule FSL.BlockMonitorTest do
  ...
  setup do
    ...
    on_exit(fn -> FSL.Monitor.clear(slot) end)
    {:ok, slot: slot}
  end

  defp row(slot) do
    Enum.find(FSL.Monitor.calls(), &(&1.slot == slot))
  end
  ...
  defp start_scenario(module, slot) do
    ...
    spawn(fn ->
      send(test, {:outcome, FSL.Runner.run_instance(module, slot_id: slot)})
    end)
  end
