defmodule FSL.StayBackTest do
  ...
  test "an on_events deadline is absolute and its remainder never goes negative" do
    assert FSL.Machine.deadline(:infinity) == :infinity
    assert FSL.Machine.remaining_timeout(:infinity) == :infinity

    deadline = FSL.Machine.deadline(1_000)
    assert FSL.Machine.remaining_timeout(deadline) in 990..1_000
    assert FSL.Machine.remaining_timeout(FSL.Machine.deadline(-5)) == 0
  end
  ...
  test "the runner records the state it came from" do
    ctx = FSL.Host.Default.build_context(label: "alice")
    assert ctx.laststate == nil
    assert FSL.Context.get(FSL.Context.put(ctx, :laststate, :calling), :laststate) == :calling
  end
  ...
  defp wait_for_event(event, attempts) do
    if Enum.any?(FSL.Monitor.calls(), &(&1.event == event)) do
      true
    else
      Process.sleep(10)
