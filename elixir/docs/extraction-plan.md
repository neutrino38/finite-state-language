...
looks like afterwards. §4.12 keeps the ledger — what this work makes more
...
2. **The dependency footprint becomes a public promise.** The TypeScript sibling
   ...
   that line — the engine needs nothing but `Logger` — provided `FSL.HTTP` (which
   needs `Req`) declares it `optional: true` (§4.9).
...
| `SIP.Scenario.SequenceJournal` (`dsl/…Journal.ex`) | 129 | `FSL.Journal` | pure; names the renderer, which becomes a host choice (§4.8) |
...
**Total moving: ~2 760 lines**, of which `FSL.Machine` and `FSL.Runner` are the
only two carrying real coupling.

### 2.2 Splits

| Today | Generic half → FSL | SIP half stays |
|---|---|---|
| `SIP.Scenario.SequenceDiagram` (175 l) | the journal→document pipeline and the `filename/1` / secret-masking rules | the PlantUML *vocabulary*: the `elixip`/`peer`/`ms` lanes, `send_INVITE → INVITE`, `media_* → *`. See §4.8 |
| `SIP.Scenario.Monitor` (361 l) | the registry itself: slot keying, `report/6`, `note_command/2`, `subscribe/1`, the parent/child row ordering | the call-shape columns `account`, `medias`, `mediaserver`, `outbound` and their `n/a` defaults. See §4.7 — **deliberately deferred**, see §7 |

`SIP.Context` is deliberately **not** in this table. It is not split: it
...
### 4.7 Reporting — `FSL.Reporter`

`report/5` already tests `Process.whereis(SIP.Scenario.Monitor)` before
reporting, which is the seam. It becomes: FSL calls `host.reporter()` (a module
or `nil`) and, when it is not `nil`, `reporter.report(slot, label, account,
state, event, type)`. `FSL.Journal` is fed unconditionally, as today, because it
is FSL's own and is a no-op when not started.

`report_account/1` and `initial_account/1` — the `SIP.Msg.Ops.asserted_username`
reading of who a UAS instance serves — move to `SIP.FSL.Host` as
`c:account/2`, called with the context and `:initial | :subsequent`. It is the
one piece of the reporting path that reads a SIP message, and the distinction it
encodes (a UAS instance names its account once, then only the script speaks) is
...
`SIP.Scenario.Monitor` itself does **not** move in this plan — see §7, P5.

### 4.8 The sequence diagram

`FSL.Journal` moves as it is. `SIP.Scenario.SequenceDiagram` stays in elixip,
and `FSL.Journal.flush/0` asks the host for its renderer
(`c:diagram_renderer/0`, default `FSL.Diagram.Mermaid`). The reason to keep
PlantUML on the elixip side is that everything making it readable is SIP
vocabulary: three fixed lanes named `elixip` / `peer` / `ms`, `send_INVITE`
rendered as an `INVITE` arrow, `send_auth_REGISTER` as `REGISTER (auth)`,
`media_play` as `play`. A generic renderer that knew none of that would produce
a worse diagram than the one we have.

The generic half that *does* move is small and worth moving: `filename/1`,
`safe_pid/1`, and the secret masking (`:passwd`, `:password`, `:ha1`, `:ha1b`
rendered as `****`) — a rule about not writing secrets to disk, which should not
be re-derived per renderer.

FSL then owes hex users a default renderer, and the obvious one is Mermaid: it
is what the TypeScript sibling ships (`toMermaid()`), it renders in GitHub and
in Artifacts without a toolchain, and writing it is ~80 lines against the same
journal. New code, listed as such in P4.

### 4.9 `FSL.HTTP` and `Req`

`HTTP.Session` has exactly two SIP references: `SIP.Scenario.Monitor.note_command`
(→ the reporter, §4.7) and `SIP.Context.set(ctx, :lasterr, :ok)` (→
...
| how a run is drawn | PlantUML, SIP lanes | `c:diagram_renderer/0` (§4.8) |
...
| `sbb_monitor_test.exs` | 7 | *none* | FSL package (follows the monitor, §7 P5) |
| `sequence_diagram_test.exs` | 10 | *none* | split with the renderer (§4.8) |
...
| `scenario_monitor_call_shape_test.exs` | 9 | throughout | stays |
...
Roughly **90 tests follow the code** into the package and **108 stay** as the
proof that the host wiring still works.
...
9. **Where a compile error points.** `stay` outside an `on_events`, `sbb_fsm`
   ...
   `FSL.Machine` would undo the reason those checks exist (§4.12). One test per
   check, asserting the reported file, and they must still pass in P3.

Nine tests, none of them large. They are the acceptance criteria of P1: the
...
- **`SIP.Scenario.Monitor`'s registered name**, which `Kelix.Application` lists
  in its supervision tree and `Process.whereis/1` looks up;
...
Write the nine tests of §5.2 against today's implementation. Green suite in
isolation is the bar (see Elixip's `CLAUDE.md` on the order-dependent files).
**Exit criterion:** the nine pass, and each one fails if its rule is
...
8. the reporter and `c:account/2`;
9. `c:diagram_renderer/0`.
...
the state machine", top to bottom, which is the readability gain §4.12 claims
...
Create the Mix project in `../finite-state-language/elixir` (app `:fsl`), move
the eight modules and the ~90 tests, add `FSL.Test.Host` and `FSL.Host.Default`.
...
Write `FSL.Diagram.Mermaid` (§4.8) — the package must ship a renderer of its
own. Reconcile the language with [the spec](../../spec/fsl-js-ts.md) clause by
clause and record every deliberate divergence *in the spec*, which is the
arbiter between the two
implementations: the pending queue exists in TS and not on the BEAM, `stay` and
`goto back` need their Elixir semantics stated, and the SBB return contract has
to be checked against `fx.sbb` / `fx.sbbReturn`.
...
### P5 — the monitor (separate, later)

`SIP.Scenario.Monitor` splitting into a generic registry plus SIP columns is a
change with three consumers outside FSL — elixipp's live view,
`Kelix.InstancePool`'s row joining, `Kelix.Control.subscribe_monitor/1` — and a
registered process name in a supervision tree. It is not on the critical path:
P1's reporter seam already lets FSL run with no monitor at all, and a hex user
gets a working package without one. Doing it with the extraction would put the
riskiest consumer-facing change in the same release as the riskiest internal
one. Deferred on purpose, with `sbb_monitor_test.exs` following it.
...
### 8.4 How much of the monitor is FSL's

Deferred to P5 (§7), but the question to answer then is whether the generic
registry keeps an open `extra: %{}` map for host columns — which would let
elixipp's view and `Kelix.InstancePool` keep working unchanged — or whether the
host subclasses the registry. The first is less code and less clever.
